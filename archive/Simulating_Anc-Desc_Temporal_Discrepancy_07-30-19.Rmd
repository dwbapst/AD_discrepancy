...
In both cases, although the relationships are very noisy, we can see that increased overlap leads to larger discrepancies between the ancestor and the descendant. But we'd loosely always expect that relationship from first principles: how much does this match what D&A's model tells us to expect? For example, remember D&A's figure 3 we recreated above?
...
We can then superimpose our curve our empirically-derived cumulative distribution of observing a discrepancy greater than `T_d` with the probability estimates from the cumulative function given in Equation 5b of D&A.
...
xlims <- c(
  0.8, max(c(T_o, AD_overlap_uniq))
  )

ylims <- c(
  0, 
  max(c(P_H_diff_greater, cumulAbove))
  )
...
An alternative way of looking at the results of these simulations might be to ask how often we would expect to see a discrepancy of 0.8 Ma or greater in a fossil record with this set of generating parameters. To do this, first we'll need the cumulative distribution for the temporal offset itself, rather than for achieving a particular discrepancy at different values of range overlap (as D&A approached the issue).

```{r echo = FALSE, dev='png'}

# cumulative probability distribution

AD_diff_uniq <- sort(unique(AD_diff_all))

cumulAbove <- sapply(AD_diff_uniq, function(x)
  sum(AD_diff_all > x) / length(AD_diff_all)
  )

plot(
  -AD_diff_uniq, 
  cumulAbove,
	xlab = "Desc. FAD - Anc. FAD",
	ylab = "Cumulative Proportion of Runs",
	main = "Empirical Cumulative Distribution Function of Age Offset"
  )

abline(v = 0.8, lty=5, col = "gray", lwd=2)

# what value is closest to 0.8
closeDiff_T_d <- which(AD_diff_uniq > 0.8)[1]

```

A nice smooth curve. Note that age offset is plotted so that the pairs with earlier appearing descendants have *positive* offsets, not negative. The values displayed are the cumulative probability of observing an offset between descendant and ancestor first-appearances as large as the values on the horizontal axis. The vertical line is our discrepancy value of interest, 0.8 Ma. The cumulative probability we can estimate for that value is:

```{r}
# the value of offset closest to 0.8 Ma
AD_diff_uniq[closeDiff_T_d]

# the cumulative probability of observing a discrepancy at least as large as the above
cumulAbove[closeDiff_T_d]

# the odds of observing such are thus, 1 in...
cumulAbove[closeDiff_T_d]/1
```
